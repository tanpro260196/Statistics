...
//        public static Config config = new Config();
		...
		private static void TimeSaverOnElapsed(object sender, ElapsedEventArgs elapsedEventArgs)
		{
			foreach (var player in TShock.Players)
				...
				{
					database.UpdateTime(player.Account.ID, TimeCache[player.Index]);
		...
		{
			database.CheckUpdateInclude(args.Player.Account.ID);
            database.UpdateKillingSpree(args.Player.Account.ID, 0, 0, 0);
		}
      ...
      if (TShock.Players[args.Who].Account != null)
        KillingSpree.ClearBlitzEvent(TShock.Players[args.Who].Account.ID);
			if (PlayerKilling.ContainsKey(TShock.Players[args.Who]))
				PlayerKilling.Remove(TShock.Players[args.Who]);

			if (TShock.Players[args.Who].Account != null && TShock.Players[args.Who].IsLoggedIn)
			{
				database.UpdateTime(TShock.Players[args.Who].Account.ID, TimeCache[args.Who]);
				TimeCache[args.Who] = 0;
			}
		}
