		...
		private static bool HandleNpcEvent(GetDataHandlerArgs args)
		{
			...
			if (IsTargeting(Main.npc[npcId]))
			{
				...
				if (hitDamage > Main.npc[npcId].life && Main.npc[npcId].active && Main.npc[npcId].life > 0)
				{
					...
					{
                        Statistics.database.UpdateKillingSpree(player.Account.ID, 1, 0, 0);
						Statistics.database.UpdateKills(player.Account.ID, KillType.Mob);
                        KillingSpree.SendKillingNotice(player.Name, player.Account.ID, 1, 0, 0);
						...
						Statistics.database.UpdateMobDamageGiven(player.Account.ID, player.Index);
					}
					//a boss kill
					else
					{
                        Statistics.database.UpdateKillingSpree(player.Account.ID, 0, 1, 0);
                        Statistics.database.UpdateKills(player.Account.ID, KillType.Boss);
                        KillingSpree.SendKillingNotice(player.Name, player.Account.ID, 0, 1, 0);
                        SpeedKills.PlayerKill(index); 
                        Statistics.SentDamageCache[player.Index][KillType.Boss] += Main.npc[npcId].life;
						Statistics.database.UpdateBossDamageGiven(player.Account.ID, player.Index);
					}

					//Push player damage dealt and damage received as well
					Statistics.database.UpdatePlayerDamageGiven(player.Account.ID, player.Index);
					Statistics.database.UpdateDamageReceived(player.Account.ID, player.Index);
					Statistics.database.UpdateHighScores(player.Account.ID);
				}
				else
				{
...
//            Console.WriteLine("player  " + player.User.ID);
            if (Statistics.PlayerKilling[player] != null)
			{
				...
				{
                    Statistics.database.UpdateKillingSpree(Statistics.PlayerKilling[player].Account.ID, 0, 0, 1);
                    Statistics.database.UpdateKills(Statistics.PlayerKilling[player].Account.ID, KillType.Player);
                    KillingSpree.SendKillingNotice(player.Name, player.Account.ID, 0, 0, 1);
                    SpeedKills.PlayerKill(index); 
                    Statistics.database.UpdateHighScores(Statistics.PlayerKilling[player].Account.ID);
					Statistics.database.UpdatePlayerDamageGiven(Statistics.PlayerKilling[player].Account.ID,
						Statistics.PlayerKilling[player].Index);
					Statistics.database.UpdateDamageReceived(Statistics.PlayerKilling[player].Account.ID,
						Statistics.PlayerKilling[player].Index);
				}
				Statistics.PlayerKilling[player] = null;
 			}

			Statistics.database.UpdateDeaths(player.Account.ID);
			Statistics.database.UpdatePlayerDamageGiven(player.Account.ID, player.Index);
			//update all received damage on death
			Statistics.database.UpdateDamageReceived(player.Account.ID, player.Index);
			Statistics.database.UpdateHighScores(player.Account.ID);

            Statistics.database.CloseKillingSpree(player.Account.ID);
            KillingSpree.ClearBlitzEvent(player.Account.ID);
